...
<head>
  ...
  <meta name="theme-color" content="#0A1936">
  ...
  <!-- SweetAlert2 CDN for modern custom alerts without URL header -->
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; user-select: none; -webkit-touch-callout: none; -webkit-user-select: none; }
    ...
    @keyframes floatIcon { 0%, 100% { transform: translateY(0); filter: drop-shadow(0 0 8px rgba(0, 210, 255, 0.8)); } 50% { transform: translateY(-3px); filter: drop-shadow(0 0 12px rgba(168, 85, 247, 0.9)); } }
    #globalTitleBar .bar-center { display: flex; flex-direction: column; align-items: center; }
    #globalTitleBar .bar-title { font-family: 'Orbitron', sans-serif; font-weight: 900; font-size: 18px; background: linear-gradient(90deg, #00d2ff, #a855f7, #00ff66, #00d2ff); background-size: 300% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: 1.5px; animation: shineText 3s linear infinite; }
    @keyframes shineText { 0% { background-position: 0% center; } 100% { background-position: 300% center; } }
    #globalTitleBar .bar-sub { font-family: 'Rajdhani', sans-serif; font-weight: 400; font-size: 9px; letter-spacing: 3px; color: rgba(255,255,255,0.8); text-transform: uppercase; margin-top: -1px; text-shadow: 0 0 5px rgba(0,210,255,0.5); }
    ...
    .app-logo { width: 150px; max-width: 75%; height: auto; margin: 0 auto 15px auto; display: block; filter: drop-shadow(0 0 15px rgba(0,210,255,0.9)); -webkit-user-drag: none; user-drag: none; pointer-events: none; }
    ...
    /* Customizing SweetAlert style to match dark theme */
    .swal2-popup {
    ...
    .swal2-confirm {
      background: linear-gradient(90deg, #0055ff, #00d2ff) !important;
...
(function() {
  ...
  // Default link updated with your provided Telegram URL
  var defaultTelegramLink = 'https://t.me/+iuwzgCxozp5hNDg1';
  var telegramDeepLink = defaultTelegramLink;
  var telegramWebLink = defaultTelegramLink;

  // Helper function for custom popups without URL headers
  ...
  database.ref('config').on('value', function(snapshot) {
    ...
    if (config) {
      telegramDeepLink = config.telegramDeep || defaultTelegramLink;
      telegramWebLink = config.telegramWeb || defaultTelegramLink;
  ...
  loginBtn.addEventListener('click', function(e) {
    ...
    if (!u || !p) { 
      showAlert('warning', 'Notice', 'Please enter both Username and Password!'); 
      return; 
    }
    ...
    database.ref('auth').once('value').then(function(snapshot) {
      ...
      loginBtn.innerText = 'LOGIN';
      showAlert('error', 'Error', 'Access Denied / Connection Error!');
    });
  });
  ...
  verifyBtn.addEventListener('click', function(e) {
    ...
    if (!code) { 
      showAlert('warning', 'Notice', 'Please enter the Security Code!'); 
      return; 
    }
    ...
    database.ref('auth').once('value').then(function(snapshot) {
      ...
      if (authData && code === authData.secCode) {
        ...
        showAlert('error', 'Verification Failed', 'Invalid Security Code!');
        secCodeInput.value = '';
      }
    }).catch(function(error) {
      verifyBtn.innerText = 'VERIFY ACCESS';
  ...
  // Updated pkgBtn handler to resolve ERR_UNKNOWN_URL_SCHEME in WebViews
  pkgBtn.addEventListener('click', function(e) {
    e.preventDefault();
    
    var rawUrl = telegramWebLink || telegramDeepLink || defaultTelegramLink;

    if (rawUrl.includes('tg://join?invite=')) {
      var inviteCode = rawUrl.split('tg://join?invite=')[1];
      rawUrl = 'https://t.me/+' + inviteCode;
    } else if (rawUrl.includes('tg://resolve?domain=')) {
      var domain = rawUrl.split('tg://resolve?domain=')[1];
      rawUrl = 'https://t.me/' + domain;
    } else if (rawUrl.startsWith('tg://')) {
      rawUrl = defaultTelegramLink;
    }
