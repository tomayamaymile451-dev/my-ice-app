...
<script>
  // Custom Alert Modal Function (Url-less alert popup)
  ...
  try {
    ...
    console.log("Firebase Init Warning: ", e);
  }
  ...
  loginBtn.addEventListener('click', function(e) {
    ...
    if (!u || !p) { 
      showCustomAlert('⚠️ Please enter both Username and Password!'); 
      return; 
    }
  ...
  // Security Verify Click Handler
  verifyBtn.addEventListener('click', function(e) {
    e.preventDefault();
    var code = secCodeInput.value.trim();

    if (!code) { 
      showCustomAlert('⚠️ Please enter the Security Code!'); 
      return; 
    }
  ...
  // See Packages / Telegram Link Click Handler
  pkgBtn.addEventListener('click', function(e) {
    e.preventDefault();
    var link = telegramWebLink;
    if (!link) {
      // Fallback redirect if database is slow
